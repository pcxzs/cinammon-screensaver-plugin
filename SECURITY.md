...
| Main-thread hangs | A blocked main loop freezes a session that holds the input grabs | No GStreamer call that waits on streaming threads runs on the main thread (NULL state changes and flushing seeks run on worker threads). The UPower connection is asynchronous. |
